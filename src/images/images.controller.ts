...
async uploadPostImages(
  ...
  files.forEach((file, index) => {
    ...
    sharp(file.buffer)
      .resize(1200, 800, { // مقاس مناسب للبوستات
        fit: 'contain',
        background: { r: 0, g: 0, b: 0, alpha: 0 },
      })
      .png() 
      .toFile(resolve(process.cwd(), 'uploads', 'posts') + `/${fileName}`); 

    if (index === 0 && !cover) {
      // أول صورة كـ cover للبوست
      this.service.setPostCover(+ref, +fileName);
    }
...
async getPostImages(
  ...
  const images = await this.service.findManyParams({ scope: 'posts', ref });
  
  return images.map(img => ({
    id: img.id,
    url: `/images/posts/${ref}/${img.id}`,
    mime: img.mime,
  }));
}
